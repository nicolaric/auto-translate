const createKeyDialog = $("#create-key-dialog")[0];
const deleteKeyDialog = $("#delete-key-dialog");
const keySubmit = $("#create-key-submit");
const createApiKeyButton = $("#create-api-key-button");
...
function prepareKeyDeletion(id) {
    ...
    $("#delete-key-button").click(async (event) => {
...
$(document).ready(async function() {
    createApiKeyButton.click(prepareKeyCreation);

    keySubmit.click((event) => {
        event.preventDefault();
        const name = $("#key-name").val();
        createKeyDialog.hide();
        createKey(name || "Secret Key");
    });
